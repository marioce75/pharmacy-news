...
async function loadBodies() {
  const filePath = path.join(__dirname, '..', 'js', 'articles.js');
  try {
    ...
    // js/articles.js doesn't exist (deleted after static-to-Node conversion)
  }
}
...
async function migrate() {
  ...
  // Check if articles already exist — skip seeding if so
  const existing = await store.listArticles('approved');
  if (existing.length > 0) {
    console.log(`Content already exists (${existing.length} approved articles). Skipping seed.`);
    return;
  }
