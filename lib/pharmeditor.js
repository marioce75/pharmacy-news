...
const { extractArticleBody } = require('../scrapers/base-scraper');
...
async function generateArticleBody(client, article) {
  let sourceContent = '';
  if (article.source_url) {
    try {
      sourceContent = await extractArticleBody(article.source_url);
    } catch {
      sourceContent = '';
    }
  }
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 4096,
    temperature: 0.4,
    system: prompts.BODY_SYSTEM_PROMPT,
    messages: [{
      role: 'user',
      content: prompts.BODY_USER_PROMPT(article, sourceContent)
    }]
  });
...
async function analyze(articleIds) {
  ...
  try {
    ...
    // Process in batches — scoring only; bodies are generated on approve
      ...
      try {
        ...
        for (const article of batch) {
          ...
          if (enrichment) {
            ...
            console.warn(`  [PHARMEDITOR] No score result for article ${article.id}`);
          }

          await store.writeArticle('pending', article.id, article);
          if (enrichment) scored++;
        }
      } catch (err) {
...
async function writeBody(articleId) {
  const client = getClient();
  if (!client) {
    throw new Error('ANTHROPIC_API_KEY not configured');
  }

  const article = await store.readArticle('pending', articleId);
  const body = await generateArticleBody(client, article);
  if (!body || body.trim().length < 100) {
    throw new Error('Body generation returned empty or too-short content');
  }
  article.body = body;
  await store.writeArticle('pending', articleId, article);
  console.log(`  [PHARMEDITOR] Body written for: ${article.title.slice(0, 60)}`);
  return article;
}
